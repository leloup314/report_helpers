...
def plot(data, x_label=None, y_label=None, title=None, output_pdf=None,
    ...
    with PdfPages(output_pdf) as out:
        ...
        data_labels = tmp_data['label'] if 'label' in tmp_data.keys() else ['Data' if i == 0 else 'Data %i' % (i + 1)
                ...
                for i, arg in enumerate(fit_args):
                    ...
                    if 'units' in tmp_fit.keys():
                        if tmp_fit['units'][i] is not None:
                            add_str = spacer + r'$%s = ' % arg + '(' + con_fit % fit_parameters[i] + ' \pm ' + con_err % \
                                                                                                               fit_param_errors[
                                                                                                                   i] + ')\ %s$' % \
                                                                                                                        fit[
                                                                                                                            'units'][
...
def example_plot():
    ...
    y2 = [(a * i ** 0.5 + b) + (a * i ** 0.5 + b) * np.random.uniform(-noise_factor, noise_factor) for i in
          range(1, 100)]
    y3 = [(a * np.exp(b * i)) + (a * np.exp(b * i)) * np.random.uniform(-noise_factor, noise_factor) for i in
          range(1, 100)]
    ...
    fit4 = {'data': [[x, y1, y1_err], [x, y2, y2_err]], 'func': [func1, func2],
            'label': ['$f(x)=a\cdot x^{b}$', '$f(x)=a\cdot\sqrt{x} + b$'], 'start_parameters': [(1, 1e-4), (1, 1e-4)],
            'precision': [3, 4], 'zorder': 10}
